...
file_name = 'app\data\knn_model.pkl'
with open(file_name, 'rb') as file:
    loaded_model = pickle.load(file)

# Load saved data
merged_df = pd.read_csv('./app/data/merged_df.csv')
ratings_df = pd.read_csv('./app/data/ratings.csv')

# Import required data for the second app
...
imdb_links = pd.read_csv('./app/data/links.csv', dtype={'movieId': 'str', 'imdbId': 'str', 'tmdbId': 'str'})
...
app_mode = st.sidebar.selectbox("Choose the app", ["Movie Recommender", "Genre-Based Recommender"])
...
elif app_mode == "Genre-Based Recommender":
    # Genre-Based Recommender App
    ...
    with st.form(key='form'):
        ...
        for i, col in enumerate(cols):
            ...
            with row1_1:
                ...
                st.image(movie_image_links[i], width=250)

            # Second will have the rating radio form
            with row1_2:
                user_movie_rating = st.radio(movie_to_rate_title, ratings_options, key=i)
